...
export async function selectDatasource(context: ExtensionContext): Promise<DataSource | null> {
...
export async function removeDatasource(context: ExtensionContext): Promise<string | null> {
    const dataSources = ConnectionManager.getDataSources().map(dataSource => ({ label: dataSource.getName() }));
