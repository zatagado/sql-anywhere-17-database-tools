...
import { inject, useTemplateRef, type CSSProperties, type VNodeRef } from 'vue';
import { formatSqlDataType } from '../../utils.ts';
...
const enableTooltip = inject('enableTooltip');
...
<template>
    <th :ref="element" class="results-header-cell">
        <button ref="tooltip-hover" type="button" @click="onSortClick">
            ...
            <Tooltip 
                v-if="enableTooltip !== false"
                :hover-element="() => tooltipHoverElement" 
                :text="formatSqlDataType(column)" 
            />
        </button>
        <div
            class="resize-handle"
