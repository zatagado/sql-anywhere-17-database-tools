...
2. In **VS Code** or a related fork, install the extension using either method:
...
3. Open a `.sql` file → run **Execute SQL** from the editor toolbar button, or **Alt**+click that button for **Execute SQL with Datasource**, or run either command from the Command Palette.
...
| Execute SQL | Run the active **SQL** editor’s selection (if any) or full document; show results in the webview. |
| Execute SQL with Datasource | Pick a datasource, then run SQL like **Execute SQL** (active editor must be **SQL**). |
